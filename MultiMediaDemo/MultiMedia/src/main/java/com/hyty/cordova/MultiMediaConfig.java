...
public class MultiMediaConfig {
    ...
    public static final String SELECT_CAMERA_FILE_PATH = null;//所有图片
                ...
                .setCompressListener(new OnCompressListener() { //设置回调
                    ...
                    public void onSuccess(File file) {
                      /*  File mFromFile = new File(formFilesPath.get(p));
                        ...
                        if(!mToFile.exists()){
                            String endName = mToFile.getPath().split("\\.")[1];
                            String newName = mMediaConfig.getFileSavedPath() + "/" + new Date().getTime() + "." + endName;
                            mToFile = new File(newName);
                            boolean flag =  FileUtils.copyFile(mFromFile.getPath(),mToFile.getPath());
                           System.out.println(flag);*/


                   /*         // 其次把文件插入到系统图库
                            try {
                                MediaStore.Images.Media.insertImage(mApplication.getContentResolver(),
                                        mToFile.getAbsolutePath(), mToFile.getName(), null);
                            } catch (FileNotFoundException e) {
                                e.printStackTrace();
                            }*/
                            // 最后通知图库更新
                            mApplication.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(new File(file.getPath()))));
//                        }
...
//    }
     ...
     * @return 新命名后的文件
     */
    private File rename(String newName) {
        //String startName = newName.split("\\.")[0];
        String endName = newName.split("\\.")[1];
        return new File(mMediaConfig.getFileSavedPath() + "/" + new Date().getTime() + "."+endName);
