...
//    private static final String TAG = "JCameraView";
    ...
    //闪关灯状态
    private static final int TYPE_FLASH_AUTO = 21;
    private static final int TYPE_FLASH_ON = 22;
    private static final int TYPE_FLASH_OFF = 23;
...
//        mTV_Flag.setText(TextUtils.isEmpty(mMultiMediaConfig.getFlagText()) ? "" : mMultiMediaConfig.getFlagText());
        ...
        String saveFlashState = ACache.get(mContext).getAsString("flashState");
        Log.d("测试", "进入页面，读取到的code = " + saveFlashState);
        if (!TextUtils.isEmpty(saveFlashState)) {
            type_flash = Integer.valueOf(saveFlashState);
            Log.d("测试", "读取到的code不为空，赋给全局的type_flash");
        }
        setFlashRes();
        mFlashLamp.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.d("测试", "点击前:" + type_flash);
                type_flash++;
                if (type_flash > 23) {
                    type_flash = TYPE_FLASH_AUTO;
                }
                Log.d("测试", "点击后:" + type_flash);
                setFlashRes();
            }
        });
...
//                    mTV_Flag.setText("广安东站-南充 1766公里999米 2017-11-14 10:03:55 ");
                });
    }
...
//        }
    ...
    private void setFlashRes() {
        ...
        ACache.get(mContext).put("flashState", String.valueOf(type_flash));
        Log.d("测试", "闪光灯设置完毕， type_flash = " + type_flash);
    }
